...
public class AcervoExcel {
	...
	public ByteArrayInputStream gerarExcelEmprestimo(List<Acervo> acervo) throws IOException {
		try (Workbook workbook = new XSSFWorkbook()) {
			...
			for (Acervo e : acervo) {
				dataRow.createCell(0).setCellValue(e.getIdObra());
				dataRow.createCell(1).setCellValue(e.getNomeObra());
				dataRow.createCell(2).setCellValue(e.getIsbn());
				dataRow.createCell(3).setCellValue(e.getStatus().toString());
				dataRow.createCell(4).setCellValue(tratamentoDatas.retornaDataCorreta(e.getDtInclusaoObra()));
			}
